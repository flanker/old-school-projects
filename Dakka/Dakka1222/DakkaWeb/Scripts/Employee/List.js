...
Ext.onReady(function() {
    ...
    var formEdit = new Ext.FormPanel({
        ...
        items: [{ id: 'Edit_Code', fieldLabel: 'Code', name: 'Code', allowBlank: false },
                ...
                { id: 'Edit_Email', fieldLabel: 'Email', name: 'Email', allowBlank: false, vtype: 'email' },
        ...
        if (dialogResult == "yes") {
            ...
            Ext.Ajax.request({
                ...
                success: function(response, options) {
                    markRemoving.hide();
                    Ext.flanker.showResult(response.responseText);  // 处理返回结果
        ...
        if (formAddNew.getForm().isValid())        //表单数据验证
        {
            formAddNew.getForm().submit({          //提交表单
                ...
                success: function(re, v) {
                    Ext.flanker.showResult(v.response.responseText);  // 处理返回结果
