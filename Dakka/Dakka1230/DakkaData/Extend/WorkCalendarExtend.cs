...
namespace DakkaData
{
    partial class WorkCalendar
    {
        ...
        public static int GetAllCount()
        {
            ...
            var result = db.WorkCalendar.Count();
        ...
        public static string GetNavPages(long ID)
        {
            ...
            var xfirst = db.WorkCalendar.OrderBy(wc => wc.ID);
            if (xfirst.Count() > 0)
            {
                first = xfirst.First().ID.ToString();
            }

            var xprev = db.WorkCalendar.Where(wc => wc.ID < ID).OrderByDescending(wc => wc.ID);
            if (xprev.Count() > 0)
            {
                prev = xprev.First().ID.ToString();
            }

            var xlast = db.WorkCalendar.OrderByDescending(wc => wc.ID);
            if (xlast.Count() > 0)
            {
                last = xlast.First().ID.ToString();
            }

            var xnext = db.WorkCalendar.Where(wc => wc.ID > ID).OrderBy(wc => wc.ID);
            if (xnext.Count() > 0)
            {
